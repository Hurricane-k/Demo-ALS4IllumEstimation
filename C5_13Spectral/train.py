...
def vald_net(net, 
    ...
    copy and paste from original C5 github
...
def train_net(
        ...
        device,
        use_spec = False,
        input_size = 64, # the  parameters of net
        learn_g=True, # the  parameters of net
        data_num=7, # the  parameters of net
        folds = 3,
        ...
        l2reg=0.00001,
        epochs=60,
        increasing_batch_size=False,
        model_name = 'C5_Spectral',
        optimizer_algo='Adam',
        ...
        save_cp=True,
        chkpoint_period=10):
    """ copy and paste from initial C5 github

    Args: (updated)
        use_spec: train with 13-channel ASL info
            (bool) True: with / False: wo

    Raises:
        NotImplementedError: _description_
    """

    if load_hist != True: # force change
        load_hist = True

    dataset_name = (dir_img.split('/')[-1] 
                    if dir_img.split('/')[-1] != '' 
                    else dir_img.split('/')[-2])
    ...
    dir_checkpoint = 'checkpoints_model/'+dataset_name+'/'

    # smoothness Sobel filters (used to extract edge info?)
    ...
    for fold in range(folds):
        ...
        if use_tb:  
            if folds > 1:
                writer = SummaryWriter(
                    log_dir='runs/'+ dataset_name + '/' + model_name + f'_fold_{fold}',
                    comment=f'LR_{lr}_BS_{batch_size}')
            else:
                writer = SummaryWriter(
                    log_dir='runs/'+ dataset_name + '/' + model_name,
                    comment=f'LR_{lr}_BS_{batch_size}')
        ...
        for epoch in range(epochs):
            ...
            with tqdm(total=len(train), 
                ...
                for batch in train_loader:
                    ...
                    if use_spec:
                        ...
                        predicted_ill, P, F, B, G = net(histogram, 
                                                        model_in_N=model_histogram,
                                                        ASL_info = spec_info)
                    else:
                        predicted_ill, P, F, B, G = net(histogram, 
                    ...
                    s_loss_B = smoothness_factor_B * (torch.mean(
                        torch.nn.functional.conv2d(
                            ...
                            u_variation, stride=1) ** 2) + torch.mean(
                                torch.nn.functional.conv2d(
                                    torch.unsqueeze(B, dim=1),
                    ...
                    s_loss_F_chroma = (torch.mean(
                        torch.nn.functional.conv2d(
                            torch.unsqueeze(F_chroma, dim=1), 
                            u_variation, stride=1) ** 2) +
                            torch.mean(
                                torch.nn.functional.conv2d(
                                    torch.unsqueeze(F_chroma, dim=1), 
                                    v_variation,
                    ...
                    s_loss_F_edges = (torch.mean(
                        torch.nn.functional.conv2d(
                            torch.unsqueeze(F_edges, dim=1), 
                            u_variation, stride=1) ** 2) +
                    ...
                    s_loss_F = smoothness_factor_F * (
                        s_loss_F_chroma + s_loss_F_edges)
                    ...
                    optimizer.zero_grad() # import steps for train
                    loss.backward() # import steps for train

                    if grad_clip_value > 0:
                        torch.nn.utils.clip_grad_value_(net.parameters(), grad_clip_value)

                    optimizer.step() # import steps for train
                    ...
                    if use_tb:
                        ...
                        if first_iteration and epoch % 2 == 0:
                            ...
                            gt_ill_img = torch.ones(bt_sz, 
                                                    3, 
                                                    200, 
                                                    200).to(device=gt.device)
                            predicted_ill_img = torch.ones(
                                bt_sz, 
                                3, 
                                200, 
                                200).to(device=gt.device)
                            gt_ill_img = gt_ill_img * gt.view(bt_sz, 
                                                            3, 1, 1)
                            predicted_ill_img = (
                                predicted_ill_img * (
                                    predicted_ill / torch.unsqueeze(
                                        torch.norm(predicted_ill, 
                                                dim=1), 
                                                dim=1)).view(bt_sz, 
                                                                3, 1, 1))
                            writer.add_images('predicted ills', 
                                            predicted_ill_img,
                                            global_step)
                            writer.add_images('gt ills', 
                                            gt_ill_img, 
                                            global_step)
                            ...
                            writer.add_images('P', 
                                            ops.vis_tensor(
                                                P, 
                                                dim=1, 
                                                scale=20 / torch.mean(P)), 
                                                global_step)
                            writer.add_images('histogram-image', 
                                            ops.vis_tensor(
                                                histogram[:, 0, :, :], 
                                                norm=True, dim=1), 
                                                global_step)
                            writer.add_images('histogram-edge', 
                                            ops.vis_tensor(
                                                histogram[:, 1, :, :], 
                                                norm=True, dim=1), 
                                                global_step)
                            writer.add_images('F-chroma (time domain)', 
                                            ops.vis_tensor(
                                                F_chroma, 
                                                dim=1, 
                                                norm=True), 
                                                global_step)
                            writer.add_images('F-edges (time domain)', 
                                            ops.vis_tensor(
                                                F_edges, 
                                                dim=1, 
                                                norm=True), 
            ...
            epoch_smoothness_loss = epoch_smoothness_loss / (
                len(train) / curr_batch_size)
            epoch_angular_loss = epoch_angular_loss / (
                len(train) / curr_batch_size)

            logging.info(
                f'Epoch loss: angular = {epoch_angular_loss},'
                f'smoothness = {epoch_smoothness_loss}')

            scheduler.step() # for every epoch

            # model validation
            if (epoch + 1) % validation_frequency == 0:

                val_score = vald_net(net=net, 
                                    loader=val_loader,
                                    use_spec=use_spec, 
                                    device=device)
        ...
        if folds > 1:  # if cv is applied
            torch.save(net.state_dict(), 
                    'models/' + 
                    f'{model_name}_'f'{dataset_name}_fold_'f'{fold + 1}.pth')
            logging.info('Saved trained model!')
        else:
            torch.save(net.state_dict(), 
                    'models/' + 
                    f'{model_name}_'f'{dataset_name}.pth')
            logging.info('Saved trained model!')
        if use_tb:
            writer.close()
...
def get_args():
    ...
    parser.add_argument('-fns', '--folds', 
                        metavar='FN', type=int, 
                        default=3,
                        help='the total number of cross validation folds', 
                        dest='folds')
    ...
    parser.add_argument('-dn', '--data-num', 
                        dest='data_num', type=int,
                        default=7, 
                        help='Number of input data to create a model')
    ...
    parser.add_argument('-ml', '--model-location', 
                        dest='model_location',
                        default=None)

    parser.add_argument('-vf', '--validation-frequency', 
                        dest='val_frq',
                        type=int, default=1, 
    ...
    parser.add_argument('-lg', '--learn-G', 
                        type=bool, default=False,
                        help='Learn G multiplier', 
                        dest='learn_g')

    parser.add_argument('-ntrd', '--training-dir-in', 
                        dest='in_trdir',
                        default='./dataset/O1_Pure',
                        help='Input training image directory')

    parser.add_argument('-nvld', '--validation-dir-in', 
                        dest='in_vldir',
                        default=None,
    ...
    parser.add_argument('-augd', '--augmentation-dir', 
                        dest='aug_dir',
                        default=None, nargs='+',
...
if __name__ == '__main__':
    ...
    net = network(input_size=args.input_size, 
                  learn_g=args.learn_g,
                  data_num=args.data_num, 
                  device=device)
    
    if args.load:
        net.load_state_dict(
        torch.load(args.model_location, 
                   map_location=device))
