...
import torch
from src import ops
from src.common_blocks import Conv
import math
...
class network(nn.Module):
  def __init__(
    ...
    # independent module to handle 13-channel ASL info
    ...
    self.upconv0.to(device=device)
    # convolution the feature fusing original and new feature map
    self.uplayer0 = Conv(13, 32, 3, 1, norm='bn')
    self.uplayer0.to(device=device)
    #########
  ...
  def forward(self, N, model_in_N, ASL_info = None):
    ...
    Args:
      ...
      model_in_N: input histogram(s) concatenated with the additional
        ...
        ASL_info: 13-channel info (13,1,1)
    ...
    if ASL_info != None: # if ASL_info is provided
      latent =+ self.uplayer0(self.upconv0(ASL_info))
    ...
    # N_after_conv = torch.fft.irfft(ops.complex_multiplication(N_fft, F_fft), 2,
    #                             onesided=False)
