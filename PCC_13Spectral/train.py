...
def main(args):
    ...
    elif args.mode == 'coarse2fine_ucc':
        model = ModelOperator(in_feature=16, neurons=8)
    elif args.mode == 'coarse2fine_ucc_spec_pcc':
        model = ModelOperator(in_feature=6, neurons=6)
    else: # semantic
    ...
    data_train = CcData(data_dir, train=True, mode=args.mode, fold_num=args.fold_num)
    train_loader = DataLoader(data_train, batch_size=args.batch_size,
                              shuffle=True, num_workers=args.num_workers, drop_last=False)
    print('============================')
    print(fr'Training set size ... {len(data_train)}')

    data_val = CcData(data_dir, train=False, mode=args.mode, fold_num=args.fold_num)
